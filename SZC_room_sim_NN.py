...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=1)  # gamma = decayrate, step_size = decaystep
...
# Training loop (use smaller grid for speed)

num_epochs = 100
for epoch in range(num_epochs):
    optimizer.zero_grad()
    q_opt = model(x)[0]  # shape: (S, J)
    pressure_field, X = compute_pressure_field_tensor(room_dim, sources, q_opt, lyd_data, grid_res=10, J=J, fs=fs)
    
    loss = 1*contrast_loss(pressure_field, X, room_dim) + 0*pressure_matching_loss(pressure_field, X, room_dim, target_db=65.0)
    if torch.isnan(loss):
        print(f"Epoch {epoch}, Contrast Loss: NaN (skipped update)")
        continue
    loss.backward()
    optimizer.step()
